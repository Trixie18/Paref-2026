...
what the browser runs. This is an **alternate UI**, not a replacement: the
Next.js app at `../frontend` still exists and still works. Both talk to the
same FastAPI backend at `../backend` over plain HTTP, and neither one talks
to Google Sheets directly - the backend is the only source of truth for
prices, totals, inventory, and access decisions.
...
2. Serve this folder as static files - no build step, just a plain file
   server. From inside `frontend-static/`:
...
`CORS_ORIGINS` env var. The Next.js app runs on `http://localhost:3000`;
this static site runs on `http://localhost:5500`. `../backend/.env` and the
root `../.env.example` both list:

```
CORS_ORIGINS=http://localhost:3000,http://localhost:5500
```

so both frontends work at once. If you copy `.env.example` to a fresh
`backend/.env`, this is already included. Restart the backend after
changing `CORS_ORIGINS` for the change to take effect.
...
values as the Next.js app's `NEXT_PUBLIC_FIREBASE_*` env vars). This mode
loads the real Firebase JS SDK from Google's CDN (`gstatic.com`) and uses
